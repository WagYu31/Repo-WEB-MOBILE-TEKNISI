...
if (isset($_GET['id_visits'])) {
    ...
    $sql = "SELECT v.*, s.nama AS nama_sales, c.nama AS nama_customer, c.no_wa AS nomor_customer, c.alamat AS alamat_customer
                FROM visits v
                LEFT JOIN sales s ON v.id_sales = s.id_sales
                LEFT JOIN cust c ON v.id_cust = c.id_cust
                WHERE v.id_visits = '$id_visits'";

    $result = mysqli_query($conn, $sql);
} else {
    echo "Parameter id_visits tidak valid.";
    exit;
}
                    ...
                    ?>
                            <div class="row">
                                <div class="col-md-12 mb-2 mt-4">
                                    <span class="label label"><?php echo $sts; ?></span>
                                    ...
                                    <?php
                                        ...
                                        ?>
                                                <div class="col-md-6 col-6 m-0 p-0 text-center">
                                                    <a href="get-mulai.php?id_visits=<?php echo $row['id_visits']; ?>" class="w-90 btn btn-info start-btn btn-block p-3">Mulai</a>
                                                </div>
                                                <div class="col-md-6 col-6 m-0 p-0 text-center">
                                                    <button class="w-90 btn btn-warning sync-btn btn-block p-3" data-toggle="modal" data-target="#rescheduleModal" data-id="<?php echo $row['id_visits']; ?>">Reschedule</button>
                                                </div>
                                            <?php
                                            } else if ($status == "on process") {
                                            ?>
                                                <div class="col-md-12 col-12 m-0 p-0 text-center">
                                                    <button class="w-90 btn btn-success finish-btn btn-block p-3" data-toggle="modal" data-target="#selesaiModal" data-id="<?php echo $row['id_visits']; ?>">Selesai</button>
                                                </div>
                                            <?php
                                            }
                                        }
                                ...
                                <div class="col-md-12 mt-0 mb-3">
                                    ...
                                    <ul class="list ms-n4 mt-2">
                                        ...
                                        <?php
                                        $selectTeknisi = "SELECT v.id_sales, v.hasil_visits, v.kode_transaksi, s.nama AS nama_sales, s.id_sales FROM visits v
                                        LEFT JOIN sales s ON v.id_sales = s.id_sales
                                        ...
                                        if (mysqli_num_rows($resTek) > 0) {
                                            while ($dataTek = mysqli_fetch_assoc($resTek)) {
                                                $ketFin = $dataTek['hasil_visits'];
                                                if ($ketFin == 'Diselesaikan oleh Admin') {
                                                    echo "";
                                                } else {
                                                    echo "<li style='list-style-type:none;'><i class='material-icons opacity-10 bg-warning text-white me-2' style='padding:3px; border-radius:50%; font-size:13px;'>chevron_right</i> " . $dataTek["nama_sales"] . "</li>";
                                                }
                                            }
                                        } else {
                                ...
                                $keterangan_cust = $row['keterangan_visits'];
                ...
                <div class="modal-content">
                    ...
                    <div class="modal-body">
                        <form id="pauseForm" method="POST" action="get-pause.php" enctype="multipart/form-data">
                            ...
                            <input type="hidden" id="kegiatanId" name="kegiatanId" value="<?php echo $id_visits; ?>">
                        </form>
                    </div>
                ...
                <div class="modal-content">
                    ...
                    <div class="modal-body">
                        <form id="selesaiForm" method="POST" action="get-selesai.php" enctype="multipart/form-data">
                            <div class="form-group">
                                <label for="permasalahan" class="font-weight-bold">Hasil Visits</label>
                                <textarea class="form-control border p-2 custom-textarea" id="editor1" name="hasil" onkeydown="handleEnter(event, 'editor1')"></textarea>
                            </div>
                            <div class="form-group">
                                <label for="solusi" class="font-weight-bold mt-2">Keterangan Tambahan</label>
                                <textarea class="form-control border p-2 custom-textarea" id="editor2" name="ketam" onkeydown="handleEnter(event, 'editor2')"></textarea>
                            </div>
                            <div class="form-group">
                            ...
                            <!-- Hidden input untuk menyimpan ID kegiatan -->
                            <input type="hidden" id="visitsId" name="visitsId" value="<?php echo $id_visits; ?>">
                        </form>
                    </div>
                    <div class="modal-footer">
                        <button type="button" class="btn btn-danger" data-dismiss="modal">Tutup</button>
