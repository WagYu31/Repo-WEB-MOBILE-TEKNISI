    ...
    if (isset($_FILES["buktiPembayaran"]) && $_FILES["buktiPembayaran"]["error"] == UPLOAD_ERR_OK) {
        ...
        if (in_array($fileExt, $allowedExt)) {
            ...
            if (move_uploaded_file($_FILES['buktiPembayaran']['tmp_name'], $uploadFile)) {
                ...
                $query = "UPDATE sedekah SET bukti_pembayaran = ?, status = 'Pending' WHERE kode_pembayaran = ?";
                ...
                if (mysqli_affected_rows($conn) > 0) {
                    header("Location: detail_pembayaran_sedekah.php?kode_pembayaran=".$kodePembayaran);
                    exit();
                } else {
