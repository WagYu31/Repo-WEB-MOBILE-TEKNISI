...
$is_filtered = isset($_GET['filter_active']);
if ($is_filtered) {
    $use_inv = isset($_GET['use_inv']) ? 1 : 0;
    $val_inv = $_GET['val_inv'] ?? 'yes';
    $use_fin = isset($_GET['use_fin']) ? 1 : 0;
    $val_fin = $_GET['val_fin'] ?? '0';
    $use_date = isset($_GET['use_date']) ? 1 : 0;
...
<head>
    ...
    <style>
        ...
        .filter-box { background: #fff; border: 1px solid #e9ecef; border-radius: 8px; padding: 15px; margin-bottom: 15px; }
                ...
                <div class="col-12">
                    <div class="card">
                        <div class="card-header p-3 pb-0">
                            <h5 class="mb-3 text-uppercase font-weight-bold">Laporan Progress Kegiatan</h5>
                                ...
                                <form method="GET" action="">
                                    ...
                                    <div class="row align-items-center">
                                        <div class="col-md-2 filter-item">
                                            <input type="text" name="cari" class="form-control px-2 border" style="font-size:12px;" placeholder="Cari nama/kode..." value="<?= htmlspecialchars($search) ?>">
                                        </div>
                                        <div class="col-md-3 filter-item d-flex align-items-center">
                                            <input class="form-check-input me-2" type="checkbox" name="use_inv" id="use_inv" value="1" <?= $use_inv ? 'checked' : '' ?>>
                                            <select name="val_inv" class="form-select form-select-sm border px-2">
                                                <option value="yes" <?= $val_inv == 'yes' ? 'selected' : '' ?>>Invoice ✅ (Ada Dokumen)</option>
                                                <option value="idea" <?= $val_inv == 'idea' ? 'selected' : '' ?>>Invoice 💡 (Dana Masuk)</option>
                                                <option value="no" <?= $val_inv == 'no' ? 'selected' : '' ?>>Invoice ❌ (Belum Ada)</option>
                                            </select>
                                        </div>
                                        <div class="col-md-2 filter-item d-flex align-items-center">
                                            <input class="form-check-input me-2" type="checkbox" name="use_fin" id="use_fin" value="1" <?= $use_fin ? 'checked' : '' ?>>
                                            <select name="val_fin" class="form-select form-select-sm border px-2">
                                                <option value="1" <?= $val_fin == '1' ? 'selected' : '' ?>>Finish ✅</option>
                                                <option value="0" <?= $val_fin == '0' ? 'selected' : '' ?>>Finish ❌</option>
                                            </select>
                                        </div>
                                        <div class="col-md-4 filter-item d-flex align-items-center">
                                            <input class="form-check-input me-2" type="checkbox" name="use_date" id="use_date" value="1" <?= $use_date ? 'checked' : '' ?>>
                                            <input type="date" name="start_date" class="form-control form-control-sm border px-2" value="<?= $start_date ?>">
                                            <span class="mx-2">-</span>
                                            <input type="date" name="end_date" class="form-control form-control-sm border px-2" value="<?= $end_date ?>">
                                        </div>
                                        <div class="col-md-1 filter-item text-end">
                                            <button type="submit" class="btn btn-info btn-sm w-100 mb-0">Filter</button>
                                        </div>
                                    </div>
                                </form>
                            </div>
                            ...
                            <div class="table-responsive">
                                <table class="table table-hover align-middle mb-0">
                                    ...
                                    <tbody>
                                        ...
                                        if (!empty($search)) {
                                            $filter_sql .= " AND (c.nama LIKE ? OR k.kode LIKE ? OR k.kegiatan LIKE ? OR k.keterangan LIKE ?)";
                                            $search_param = "%$search%";
                                        ...
                                        if ($use_inv) {
                                            if ($val_inv == 'yes') {
                                                $filter_sql .= " AND EXISTS (SELECT 1 FROM pendapatan_kegiatan WHERE kode = k.kode LIMIT 1)";
                                            } elseif ($val_inv == 'idea') {
                                                $filter_sql .= " AND NOT EXISTS (SELECT 1 FROM pendapatan_kegiatan WHERE kode = k.kode LIMIT 1) AND k.paid REGEXP '^[0-9]+$'";
                                            } elseif ($val_inv == 'no') {
                                                $filter_sql .= " AND NOT EXISTS (SELECT 1 FROM pendapatan_kegiatan WHERE kode = k.kode LIMIT 1) AND (k.paid IS NULL OR k.paid = '' OR k.paid NOT REGEXP '^[0-9]+$')";
                                            }
                                        }

                                        if ($use_fin) {
                                            if ($val_fin == '1') {
                                                $filter_sql .= " AND pk.is_finish = 1";
                                            } else {
                                                $filter_sql .= " AND (pk.is_finish = 0 OR pk.is_finish IS NULL)";
                                            }
                                        }

                                        if ($use_date) {
                                            $filter_sql .= " AND DATE((SELECT MIN(jadwal) FROM kegiatan WHERE kode = k.kode)) BETWEEN ? AND ?";
                                        ...
                                        $sql_count = "SELECT COUNT(*) as total FROM kegiatan k
                                                      ...
                                                      WHERE k.deleted_at IS NULL" . $filter_sql;
                                        ...
                                        if (!empty($params)) { $stmt_main->bind_param($types, ...$params); }
                                        $stmt_main->execute();
                                        ...
                                        if ($result_main->num_rows > 0) {
                                            while ($row = $result_main->fetch_assoc()) {
                                                ...
                                                $has_invoice = !empty($row['pkeg_no_invoice']);
                                                $is_numeric_paid = preg_match('/^[0-9]+$/', trim($row['paid'] ?? ''));
                                        ?>
                                                    ...
                                                    <td class="py-3 pe-4 col-doc-info">
                                                        <div class="d-flex flex-wrap align-items-center gap-3 mb-2 progress-checkboxes">
                                                            <div class="d-flex align-items-center">
                                                                ...
                                                                <?php elseif ($is_numeric_paid): ?>
                                                                    <i class="fas fa-lightbulb text-warning" style="font-size:14px;"></i>
                                                                <?php else: ?>
                                                        ...
                                                        <div class="d-flex flex-column" id="wrapper_info_<?= $kodeTransaksi; ?>">
                                                            ...
                                                            <?php elseif ($is_numeric_paid): ?>
                                                                <div class="info-doc-box">
                                                                    <i class="fas fa-lightbulb text-warning me-1"></i> <strong>Invoice:</strong> Nominal disetorkan, belum ada nomor invoice.
                                                                </div>
                                                            <?php else: ?>
                                                                <div class="info-doc-box">
                                        ...
                                        <?php
                                            ...
                                            echo "<tr><td colspan='3' class='text-center py-5 text-secondary'>Tidak ada data ditemukan sesuai filter.</td></tr>";
                                        }
                                        $stmt_main->close();
                                        ?>
                                    </tbody>
                        ...
                        <div class="card-footer px-3 border-0 d-flex justify-content-center justify-content-lg-center">
                            <ul class="pagination pagination-sm mb-0">
                                <li class="page-item <?= ($page <= 1) ? 'disabled' : ''; ?>">
                                    <a class="page-link" href="?page=<?= $page - 1; ?><?= $qs; ?>"><i class="fas fa-angle-left"></i> </a>
                                </li>
                                <?php
                                $adjacents = 2;
                                ...
                                <li class="page-item <?= ($page >= $total_pages) ? 'disabled' : ''; ?>">
                                    <a class="page-link" href="?page=<?= $page + 1; ?><?= $qs; ?>"> <i class="fas fa-angle-right"></i></a>
                                </li>
                            </ul>
                        </div>
                        <?php endif; ?>
