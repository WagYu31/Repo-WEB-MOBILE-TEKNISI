...
if ($_SERVER['REQUEST_METHOD'] == 'POST' && isset($_POST['submit_kegiatan'])) {
    ...
    // [PERBAIKAN KRUSIAL] Tipe data untuk lat, lon, dan rad diubah menjadi 's' (string) agar cocok dengan struktur tabel VARCHAR Anda.
...
<body class="g-sidenav-show bg-gray-200">
                    ...
                    <div class="card z-index-2">
                        <div class="card-header col-9 col-md-7 p-0 position-relative mt-n4 mx-3 z-index-2 bg-transparent">
                            <div class="bg-gradient-info shadow-info border-radius-lg py-3 pe-1">
                                <div class="chart px-3"><h5 class="text-light text-bold">Tambah Kegiatan Baru</h5></div>
                            </div>
                        </div>
                        <div class="card-body">
                            <form method="POST" action="kegiatan-baru.php" id="kegiatanForm">
                                ...
                                <div class="form-group mt-2">
                                    ...
                                    <select class="form-control border p-2" id="kegiatan" name="kegiatan"><option value="survey">Survey</option><option value="pasang baru">Pasang Baru</option><option value="service">Service</option></select>
                                </div>
                                <div class="form-group mt-2">
                                    <label for="kegiatan_relasi">Terkait dengan Kegiatan (Opsional)</label>
                                    <select class="form-control border p-2" id="kegiatan_relasi" name="kegiatan_relasi" disabled><option value="">Pilih Customer Dahulu</option></select>
                                </div>
                                ...
                                <div class="form-group my-4">
                                    ...
                                    <div class="input-group mb-2">
                                        ...
                                        <button class="btn btn-outline-primary mb-0 py-1" type="button" id="gmap_search_btn">Cari di Google Maps</button>
                                    </div>
                                    <div id="map"></div>
                                    <div class="row mt-2">
                                    ...
                                    <div class="mt-2">
                                        ...
                                        <div class="d-flex align-items-center input-group">
                                            <span class="input-group">Radius (meter)</span><br>
                                            <input type="number" class="form-control border p-2" id="radius_input" value="80" aria-label="Radius dalam meter">
                                        </div>
                                    </div>
                                </div>
    ...
    document.addEventListener('DOMContentLoaded', function() {
        ...
        // --- Fungsi Dropdown & Fetch Data (Logika Riwayat Diperbaiki) ---
        ...
        function fetchCustomerData(customerId) {
            ...
            fetch(`data-kegiatan-cust.php?customer_id=${customerId}`)
                ...
                .then(data => {
                    outputDataContainer.innerHTML = /Android|webOS|iPhone/i.test(navigator.userAgent) 
                        ? '<div class="card card-body"><p class="text-center">Riwayat tidak ditampilkan di mobile.</p></div>' 
                        : data.displayHtml;
        ...
        // --- Skrip Peta dengan Pencarian Google Maps ---
        ...
        // [DIPERBAIKI] URL pencarian Google Maps
        document.getElementById('gmap_search_btn').addEventListener('click', () => {
            const query = document.getElementById('gmap_search').value;
            if (query) window.open(`https://www.google.com/maps/search/${encodeURIComponent(query)}`, '_blank');
        });
