...
<div class="col-lg-12 mt-n3 mb-4" id="loadMoreX1" style="display: block;">
    <div class="card h-100 py-3" style="border-top-left-radius:0;">
        ...
        $sql_today = "SELECT k.*, c.nama AS nama_customer, c.telp AS cust_nomor, c.alamat FROM kegiatan k LEFT JOIN customer c ON k.customer_id = c.id WHERE k.status NOT IN ('waiting', 'selesai by admin', 'selesai', 'Clear') AND DATE(k.jadwal) = ? AND k.deleted_at IS NULL ORDER BY k.jadwal ASC";
                ...
                <table class="table table-hover align-middle mb-0">
                    ...
                    <tbody>
                        ...
                        <tr>
                            ...
                            <td><p class="text-xs font-weight-bold mb-0 text-capitalize text-wrap"><?= htmlspecialchars($row['alamat'] ?? ''); ?> <button class="btn btn-secondary text-light p-0 px-1 m-0" onclick='openLocationModal(<?= json_encode($row) ?>)'><i class="material-icons" style="font-size:12px;">edit</i></button></p><p class="text-xs text-secondary mb-0 fst-italic">"<?= !empty($row["keterangan"]) ? htmlspecialchars($row["keterangan"]) : '-'; ?>"</p></td>
