        ...
        <div class="card-body p-0">
            <div class="table-responsive">
                <table class="table table-hover align-middle mb-0">
                    ...
                    <tbody>
                        <?php
                        // $sql_waiting = "SELECT k.*, c.nama AS nama_customer, c.telp AS cust_nomor, c.alamat, c.id as customer_id FROM kegiatan k LEFT JOIN customer c ON k.customer_id = c.id WHERE k.status = 'waiting' AND k.deleted_at IS NULL ORDER BY CASE WHEN DATE(k.jadwal) = CURDATE() THEN 1 WHEN DATE(k.jadwal) > CURDATE() THEN 2 ELSE 3 END ASC, k.created_at ASC";
                        $sql_waiting = "SELECT k.*, c.nama AS nama_customer, c.telp AS cust_nomor, c.alamat, c.id as customer_id FROM kegiatan k LEFT JOIN customer c ON k.customer_id = c.id WHERE k.status = 'waiting' AND k.deleted_at IS NULL ORDER BY k.created_at ASC";
                        ...
                        ?>
                            ...
                            echo "<tr><td colspan='4' class='text-center py-5'>Tidak ada data dalam waiting list.</td></tr>";
                        } ?>
                    </tbody>
                </table>
            </div>
