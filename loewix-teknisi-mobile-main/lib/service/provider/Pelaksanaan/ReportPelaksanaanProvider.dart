...
class ReportPelaksanaanProvider extends ChangeNotifier {
  final ApiPelaksanaan apiService;

  ReportPelaksanaanProvider({required this.apiService});
  ...
  //     notifyListeners();
  //
  //     uploadResponse = await apiService.updatePelaksanaan(
  //         imagesBytes, kegiatanId, teknisiId, permasalahan, solusi, keterangan, latS, lonS);
  //
  ...
  Future<String> upload(
      ...
      String? solusi,
      String? keterangan,
      // List<int>? recordFileBytes, // ← Tambahkan parameter ini
      String? ketGaransi
      ) async {
    try {
      message = "";
      ...
      uploadResponse = await apiService.updatePelaksanaan(
        ...
        solusi,
        keterangan,
        // recordFileBytes, // ← Sertakan saat pemanggilan API
        ketGaransi
      );
