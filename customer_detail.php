...
if (isset($_GET["id_cust"])) {
    ...
    // Misalnya:
    $sql = "SELECT k.id_cust, k.*, t.nama AS nama_teknisi, c.nama AS nama_customer
        FROM kegiatan k
        LEFT JOIN teknisi t ON k.id_teknisi = t.id_teknisi
        LEFT JOIN customer c ON k.id_cust = c.id_cust
        WHERE k.id_cust = $customer
        ORDER BY 
            CASE 
    ...
    <div class="container-fluid">
        <div class="row">
            ...
            <main id="content" class="mx-auto">
                <div class="container">
                    <?php
                        $queryCus = "SELECT * FROM customer WHERE id_cust = $customer";
                        $resCus = mysqli_query($conn, $queryCus);
                        $dataCus = mysqli_fetch_assoc($resCus);
                        $namaCus = $dataCus["nama"];
                    ?>
                    <h2>Data Permintaan Customer : <?php echo $namaCus; ?></h2>
                    <a href="data-customer.php" class="btn btn-secondary mb-3">
                        <i class='bx bx-arrow-back'></i> Kembali ke Daftar Customer
                    </a>
