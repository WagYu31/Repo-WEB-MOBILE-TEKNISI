        ...
        while (($rowData = fgets($fileHandle)) !== false) {
            ...
            if ($dataType == 'C') {
                ...
                foreach ($rowData as $rowDatum) {
                    switch ($rowDatum[0]) { // Update curly braces to square brackets
                        case 'C':
                        case 'X':
    ...
    public function loadIntoExisting($pFilename, PHPExcel $objPHPExcel)
    {
        ...
        while (($rowData = fgets($fileHandle)) !== false) {
            ...
            if ($dataType == 'P') {
                    ...
                    switch ($rowDatum[0]) { // Update curly braces to square brackets
                        ...
                        case 'S':
                            ...
                            for ($i = 0; $i < strlen($styleSettings); ++$i) {
                                switch ($styleSettings[$i]) { // Update curly braces to square brackets
                                    case 'I':
                ...
                foreach ($rowData as $rowDatum) {
                    switch ($rowDatum[0]) { // Mengubah {0} menjadi [0]
                        case 'C':
                        case 'X':
                            ...
                            foreach ($temp as &$value) {
                                ...
                                if ($key = !$key) {
                                    ...
                                    foreach ($cellReferences as $cellReference) {
                                        ...
                                        if ($rowReference[0] == '[') { // Mengubah {0} menjadi [0]
                                        ...
                                        if ($columnReference[0] == '[') { // Mengubah {0} menjadi [0]
                ...
                $cell->setValue(($hasCalculatedValue) ? $cellDataFormula : $cellData);
                if ($hasCalculatedValue) {
                    $cellData = PHPExcel_Calculation::unwrapResult($cellData);
                    $cell->setCalculatedValue($cellData);
                }
            } elseif ($dataType == 'F') {
                $formatStyle = $columnWidth = $styleSettings = '';
                $styleData = array();

                // Iterasi setiap elemen dalam $rowData
                foreach ($rowData as $rowDatum) {
                    switch ($rowDatum[0]) { // Mengubah {0} menjadi [0]
                        case 'C':
                        case 'X':
                        ...
                        case 'S':
                            $styleSettings = substr($rowDatum, 1);
                            // Iterasi setiap karakter dalam $styleSettings
                            for ($i = 0; $i < strlen($styleSettings); ++$i) {
                                switch ($styleSettings[$i]) { // Mengubah {$i} menjadi [$i]
                                    case 'I':
                ...
                // Set format style jika tersedia
                ...
                // Set style data jika tidak kosong
                if ((!empty($styleData)) && ($column > '') && ($row > '')) {
                    $columnLetter = PHPExcel_Cell::stringFromColumnIndex($column - 1);
                    $objPHPExcel->getActiveSheet()->getStyle($columnLetter . $row)->applyFromArray($styleData);
                }

                // Set lebar kolom jika terdefinisi
                if ($columnWidth > '') {
                ...
                foreach ($rowData as $rowDatum) {
                    switch ($rowDatum[0]) { // Mengubah {0} menjadi [0]
                        case 'C':
                        case 'X':
                            // Jika elemen dimulai dengan 'C' atau 'X', set nilai kolom
                            $column = substr($rowDatum, 1);
                            break;
                        case 'R':
                        case 'Y':
                            // Jika elemen dimulai dengan 'R' atau 'Y', set nilai baris
                            $row = substr($rowDatum, 1);
                            break;
                    }
                }
