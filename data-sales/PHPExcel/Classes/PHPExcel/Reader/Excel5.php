...
class PHPExcel_Reader_Excel5 extends PHPExcel_Reader_Abstract implements PHPExcel_Reader_IReader
{
    ...
    private function readDateMode()
    {
        ...
        if (ord($recordData[0]) == 1) {  // updated line
    ...
    private function readFont()
    {
        ...
        if (!$this->readDataOnly) {
            ...
            $underlineType = ord($recordData[10]);  // Updated line
    ...
    private function readXf()
    {
        ...
        if (!$this->readDataOnly) {
            ...
            $horAlign = (0x07 & ord($recordData[6])) >> 0;  // Updated line
            switch ($horAlign) {
                case 0:
            ...
            $wrapText = (0x08 & ord($recordData[6])) >> 3;  // Updated line
            switch ($wrapText) {
                case 0:
            ...
            $vertAlign = (0x70 & ord($recordData[6])) >> 4;  // Updated line
            switch ($vertAlign) {
                case 0:
            ...
            if ($this->version == self::XLS_BIFF8) {
                ...
                $angle = ord($recordData[7]);  // Updated line
                $rotation = 0;
                if ($angle <= 90) {
                    $rotation = $angle;
                } elseif ($angle <= 180) {
                ...
                $indent = (0x0F & ord($recordData[8])) >> 0;  // Updated line
                $objStyle->getAlignment()->setIndent($indent);

                // bit: 4; mask: 0x10; 1 = shrink content to fit into cell
                $shrinkToFit = (0x10 & ord($recordData[8])) >> 4;  // Updated line
                switch ($shrinkToFit) {
                    case 0:
                ...
                $orientationAndFlags = ord($recordData[7]);  // Updated line
    ...
    private function readStyle()
    {
        ...
        if (!$this->readDataOnly) {
            ...
            if ($isBuiltIn) {
                ...
                $builtInId = ord($recordData[2]);  // Updated line
    ...
    private function readSheet()
    {
        ...
        switch (ord($recordData[4])) {  // Updated line
            case 0x00:
        ...
        $sheetType = ord($recordData[5]);  // Updated line
    ...
    private function readDefinedName()
    {
        ...
        if ($this->version == self::XLS_BIFF8) {
            ...
            $nlen = ord($recordData[3]);  // Updated line
    ...
    private function readSst()
    {
        ...
        for ($i = 0; $i < $nm; ++$i) {
            ...
            // option flags
            $optionFlags = ord($recordData[$pos]);  // Updated line
            ++$pos;
            ...
            if ($pos + $len <= $limitpos) {
                ...
                while ($charsLeft > 0) {
                    ...
                    $option = ord($recordData[$pos]);  // Updated line
                    ++$pos;
                    ...
                    if ($isCompressed && ($option == 0)) {
                        ...
                        $len = min($charsLeft, $limitpos - $pos);
for ($j = 0; $j < $len; ++$j) {
    $retstr .= $recordData[$pos + $j] . chr(0);  // Updated line
}
$charsLeft -= $len;
$isCompressed = false;
    ...
    private function readFormula()
    {
        ...
        $isPartOfSharedFormula = $isPartOfSharedFormula && ord($formulaStructure[2]) == 0x01;  // Updated line
...
if ($isPartOfSharedFormula) {
    ...
    $this->_baseCell = PHPExcel_Cell::stringFromColumnIndex($baseCol) . ($baseRow + 1);
        ...
        if (($this->getReadFilter() !== null) && $this->getReadFilter()->readCell($columnString, $row + 1, $this->phpSheet->getTitle())) {
            ...
            if ((ord($recordData[6]) == 0) && (ord($recordData[12]) == 255) && (ord($recordData[13]) == 255)) {  // Updated lines
    ...
    private function readSharedFmla()
    {
        ...
        $no = ord($recordData[7]);  // Updated line
    ...
    private function readBoolErr()
    {
        ...
        if (($this->getReadFilter() !== null) && $this->getReadFilter()->readCell($columnString, $row + 1, $this->phpSheet->getTitle())) {
            ...
            $boolErr = ord($recordData[6]);  // Updated lines
$isError = ord($recordData[7]);  // Updated lines
...
if (!$this->readDataOnly) {
    ...
    private function readSelection()
    {
        ...
        if (!$this->readDataOnly) {
            ...
            $paneId = ord($recordData[0]);  // Updated line
    ...
    private function readHyperLink()
    {
        ...
        if (!$this->readDataOnly) {
            ...
            } elseif (ord($recordData[$offset]) == 0x03) {  // Updated line
                $hyperlinkType = 'local';
            } elseif (ord($recordData[$offset]) == 0xE0) {  // Updated line
                $hyperlinkType = 'URL';
            }
    ...
    private function readBIFF5CellRangeAddressFixed($subData)
    {
        ...
        $fc = ord($subData[4]);  // Updated line

// offset: 5; size: 1; index to last column
$lc = ord($subData[5]);  // Updated line
    ...
    private static function readRGB($rgb)
{
    ...
    $r = ord($rgb[0]); // Updated line

    // offset: 1; size: 1; Green component
    $g = ord($rgb[1]); // Updated line

    // offset: 2; size: 1; Blue component
    $b = ord($rgb[2]); // Updated line
